...
class TestPipelineOrchestrator:
    ...
    @patch('builtins.open')
    @patch('src.core.pipeline_orchestrator.Path')
    def test_load_api_config_success(self, mock_path_class, mock_open, mock_yaml_load, orchestrator):
        """Test successful API config loading."""
        mock_config = {"api": {"key": "value"}}
        mock_yaml_load.return_value = mock_config
        
        # Mock the Path chain: Path(__file__).parent.parent.parent / ...
        mock_file_path = Mock()
        mock_parent1 = Mock()
        mock_parent2 = Mock()
        mock_parent3 = Mock()
        ...
        mock_parent3.__truediv__ = Mock(return_value=mock_config_path)
        mock_config_path.exists.return_value = True
        
        with patch('src.core.pipeline_orchestrator.__file__', '/fake/path/file.py'):
    ...
    @patch('src.core.pipeline_orchestrator.Path')
    def test_load_api_config_file_not_found(self, mock_path_class, orchestrator):
