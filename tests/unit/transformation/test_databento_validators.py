...
def test_validate_timestamp_timezone_aware():
    ...
    assert validate_timestamp_timezone_aware(naive) == False
...
def test_validate_symbol_format():
    ...
    assert validate_symbol_format("CL.c.0") == True
    assert validate_symbol_format("SPY") == True
    assert validate_symbol_format("ES_H24") == True
    assert validate_symbol_format("es.fut") == True  # lowercase now allowed
...
def test_ohlcv_schema_invalid_prices():
    df = pd.DataFrame({
        ...
        "ts_event": [datetime(2024, 1, 1, tzinfo=timezone.utc)],
        "open": [100.0],
        "high": [98.0],  # High < Low should fail
        "low": [99.0],
        "close": [101.0],
        "volume": [1000]
    })
...
def test_trade_schema_negative_price():
    df = pd.DataFrame({
        "instrument_id": [1],
        "ts_event": [datetime.now(timezone.utc)],
        "price": [-100.50],  # Negative price should fail
        "size": [100],
        "side": ["A"]
    })
    with pytest.raises(pa.errors.SchemaError):
...
def test_validate_dataframe_warning_severity():
    """Test validation with WARNING severity (logs warning, returns data)."""
    df = pd.DataFrame({
        "instrument_id": [1],
        "ts_event": [datetime.now(timezone.utc)],
        "open": [100.0],
        ...
        try:
            ...
            if "ohlcv" in schema_name:
                ...
                df = pd.DataFrame({
                    "instrument_id": [1],
                    "ts_event": [datetime.now(timezone.utc)],
                    "stat_type": [1], "stat_value": [100.0], "update_action": ["A"]
                })
