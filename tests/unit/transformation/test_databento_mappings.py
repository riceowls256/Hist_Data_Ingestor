...
def test_ohlcv_transformation(rule_engine):
    ...
    result = rule_engine.transform_record(record, 'ohlcv')
...
def test_tbbo_transformation(rule_engine):
    ...
    assert result['bid_px_00'] == Decimal('100.1')
    assert result['ask_px_00'] == Decimal('100.2')
    assert result['bid_sz_00'] == 10
    assert result['ask_sz_00'] == 12
...
def test_null_fields_and_type_mismatch(rule_engine):
    ...
    result = rule_engine.transform_record(record, 'tbbo')
    assert result['bid_px_00'] is None
    assert result['ask_px_00'] is None
    assert result['bid_sz_00'] is None
    assert result['ask_sz_00'] is None
