...
class DatabentoE2ETestCase(unittest.TestCase):
    ...
    def test_ohlcv_pipeline_execution(self):
        ...
        expected_log_patterns = [
            ...
            r"TimescaleLoader.*stored successfully"
        ]
    ...
    def test_timescale_loader_compatibility(self):
        """
        Test TimescaleLoader compatibility with all Databento schema outputs.
        
        AC: 2 - TimescaleLoader reusability confirmed
        """
        print("\n🔗 Testing TimescaleLoader Compatibility...")
        
        # Test with a simple OHLCV job to verify TimescaleLoader integration
        command = [
            "python", "main.py", "ingest",
            "--api", "databento",
            "--config", self.test_config_path,
        ...
        return_code, stdout, stderr = self._execute_cli_command(command)
        self.assertEqual(return_code, 0, f"TimescaleLoader compatibility test failed")
        
        # Verify TimescaleLoader successfully handled the data
        actual_count, sample_records = self._verify_database_records('daily_ohlcv_data', 3)
        self.assertEqual(actual_count, 3, "TimescaleLoader should store all valid records")
        ...
        print("✅ TimescaleLoader compatibility verified")
